...
from kodi_six import xbmcaddon, xbmc, xbmcplugin, xbmcgui
...
__all__ = ['SimplePluginError', 'Storage', 'MemStorage', 'Addon', 'Plugin', 'Params', 'debug_exception']
...
class Addon(object):
    ...
    def __init__(self, id_=''):
        ...
        try:
            ...
            self._configdir = xbmc.translatePath(self._addon.getAddonInfo('profile'))
    ...
    def name(self):
        ...
        :return: Addon NAME, e.g. 'Example Kodi Video Plugin'
        :rtype: str
        """
    ...
    def get_localized_string(self, id_, encode=False):
        """
...
class Plugin(Addon):
    ...
    def _resolve_function(self):
        ...
        try:
            ...
            with debug_exception(self.log_error):
                ...
                try:
                    getargsspect = inspect.getargspec(action_callable).args
                except (DeprecationWarning, PendingDeprecationWarning):
                    getargsspect = inspect.getfullargspec(action_callable).args

                if inspect.isfunction(action_callable) and not getargsspect:
    ...
    @staticmethod
    def create_listing(
            listing, succeeded=True, update_listing=False, cache_to_disk=False, sort_methods=None, view_mode=None,
    ...
    def create_list_item(item):
        ...
        if major_version >= '18':
            list_item = xbmcgui.ListItem(
                label=item.get('label', ''), label2=item.get('label2', ''), path=item.get('path', ''),
    ...
    def _add_directory_items(self, context):
        ...
        xbmcplugin.endOfDirectory(self._handle, context.succeeded, context.update_listing, context.cache_to_disk)
